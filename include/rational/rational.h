...
public:
     ...
     * fractional part
     */
    typedef typename _mod<integer_type, GCD, CHKOP, Alloc, std::numeric_limits<integer_type>::is_signed>::pair_type mod_type;

    /**
     * @brief addition (@c a @b + @c b) operator
     */
    //typedef CHKOP<std::plus<T>> op_plus;
	typedef CHKOP<std::plus<int64_t>, int64_t, false> op_plus;

    /**
     * @brief subtraction (@c a @b - @c b) operator
     */
    //typedef CHKOP<std::minus<T> > op_minus;
	typedef CHKOP<std::minus<int64_t>, int64_t, false> op_minus;

    /**
     * @brief subtraction (@c a @b - @c b) operator
     */
    //typedef CHKOP<std::negate<T> > op_negate;
	typedef CHKOP<std::negate<int64_t>, int64_t, false> op_negate;
	...
	typedef CHKOP<std::multiplies<int64_t>, int64_t, false> op_multiplies;

    /**
     * @brief division (@c a @b / @c b) operator
     */
    //typedef CHKOP<std::divides<T>> op_divides;
	typedef CHKOP<std::divides<int64_t>, int64_t, false> op_divides;

    /**
     * @brief modulus (@c a @b % @c b) operator
     */
    //typedef CHKOP<std::modulus<T> > op_modulus;
	typedef CHKOP<std::modulus<int64_t>, int64_t, false> op_modulus;
	...
	integer_type numerator() const RATIONAL_NOEXCEPT {
        return m_numer;
    }
	...
	integer_type denominator() const RATIONAL_NOEXCEPT {
        return m_denom;
    }
...
private:
    ...
    static std::size_t countLeading ( IIter first, IIter last,
        ...
        //const IIter& f ( std::find_if ( first, last, std::not1 ( std::bind2nd ( std::equal_to<typename std::iterator_traits<IIter>::value_type>(), v ) ) ) );
		const IIter& f ( std::find_if ( first, last, std::find_if(begin, end, [v](const auto& x) { return x != v; }) ) );
...
Rational<T, GCD, CHKOP, Alloc>& Rational<T, GCD, CHKOP, Alloc>::reduce() {
    ...
    if ( std::not_equal_to<integer_type>() ( x, one_ ) ) {
        m_numer = RATIONAL_MOVE ( op_divides() ( m_numer, static_cast<integer_type>(x) ) );
        m_denom = RATIONAL_MOVE ( op_divides() ( m_denom, static_cast<integer_type>(x) ) );
    }
...
Rational<T, GCD, CHKOP, Alloc>::knuth_addSub ( const Rational<T, GCD, CHKOP, Alloc>& o ) {
    ...
    if ( d1 == one_ ) {
        ...
        //m_numer = RATIONAL_MOVE ( Op() ( op_multiplies() ( m_numer, o.m_denom ), op_multiplies() ( m_denom, o.m_numer ) ) );
        m_numer = Op()(op_multiplies()(m_numer, o.m_denom), op_multiplies()(m_denom, o.m_numer));
        ...
        m_denom = RATIONAL_MOVE ( op_multiplies() ( op_divides() ( m_denom, d1 ), op_divides() ( o.m_denom, d2 ) ) );
    }
