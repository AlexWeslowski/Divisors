...
private:
    Divisors() {
        ...
        */
        ...
        if (false && verbose) std::cout << "Line " << std::source_location::current().line() << std::endl;
        if (verbose) std::cout << "LEN_PRIMES = " << LEN_PRIMES << std::endl;
        if (verbose) std::cout << "isprime[46351] = " << isprime[46351] << std::endl;        
...
public:
    static Divisors& get_instance() {
        static Divisors instance;
        return instance;
    }
    ...
    std::vector<T> _rec_gen(int64_t n, const std::map<T, int>& factors, const std::vector<int64_t>& keys) {
    ...
    std::pair<T, T> _factorint_small(std::map<T, int>& factors, T n, T limit, int fail_max, T next_p) {
        if (verbose) std::cout << "_factorint_small(factors=" << to_string<T, int>(factors) << ", n = " << n << ", next_p = " << next_p << ")" << std::endl;
    ...
    bool _check_termination(std::map<T, int>& factors, T n, T next_p, int call_depth) {
        if (verbose) std::cout << "_check_termination(factors=" << to_string<T, int>(factors) << ", n = " << n << ", next_p = " << next_p << ", call_depth=" << call_depth << ")" << std::endl;

        if (n == 1 || call_depth >= MAX_RECURSION) {
            return true;
        }

        if (verbose) std::cout << "_check_termination() n < next_p * next_p (" << n << " < " << next_p << " * " << next_p << ") ? " << (n < next_p * next_p) << std::endl;
        if (verbose) std::cout << "_check_termination() n < setprimes.size() (" << n << " < " << setprimes.size() << ") ? " << (n < setprimes.size()) << std::endl;
        if (n < next_p * next_p || (n < setprimes.size() && setprimes[n])) {
            factor_cache[n] = n;
            factors[n] = 1;
            return true;
        }
        ...
        if (verbose) std::cout << "_perfect_power(n=" << n << ", next_p=" << next_p << ") returned base=" << base << ", exp=" << exp << std::endl;

        if (base == 0 && exp == 0) {
            return false;
        }

        if (verbose) std::cout << "_check_termination() base < next_p * next_p (" << base << " < " << next_p << " * " << next_p << ") ? " << (base < next_p * next_p) << std::endl;
        if (verbose) std::cout << "_check_termination() base < setprimes.size() (" << base << " < " << setprimes.size() << ") ? " << (base < setprimes.size()) << std::endl;
        if (base < next_p * next_p || (base < setprimes.size() && setprimes[base])) {
            factor_cache[n] = base;
            factors[base] = exp;
        } else {
    ...
    std::pair<T, bool> _trial(std::map<T, int>& factors, T n, const std::vector<T>& candidates) {
        if (verbose) std::cout << "_trial(factors=" << to_string<T, int>(factors) << ", n = " << n << ", candidates = " << to_string<T>(candidates) << ")" << std::endl;
    ...
    std::pair<T, int> _perfect_power(T n, T next_p, int call_depth) {
        if (verbose) std::cout << "_perfect_power(n=" << n << ", next_p=" << next_p << ", call_depth=" << call_depth << ")" << std::endl;
    ...
    std::map<T, int> factorint(T n, int call_depth) {
        if (verbose) std::cout << "factorint(n=" << n << ", call_depth=" << call_depth << ")" << std::endl;
        ...
        if (verbose) std::cout << "_check_termination(" << to_string<T, int>(factors) << ", " << n << ", " << next_p << ") returned " << check_term << std::endl;
        if (check_term) {
            return factors;
        }
        ...
        while (true) {
            ...
            if (use_trial) {
                std::vector<T> ps;
                for (T x = low; x < high_; x += 2) {
                    if (x < setprimes.size() && setprimes[x]) {
                        ps.push_back(x);
                    }
                }
                T new_n = 0;
                std::tie(new_n, found_trial) = _trial(factors, n, ps);
                if (verbose) std::cout << "_trial(n=" << n << ", ps=primerange(" << low << ", " << high_ << ")) returned " << new_n << ", " << found_trial << std::endl;
                n = new_n;
                next_p = high_;
                if (found_trial) {
                    bool check_term = _check_termination(factors, n, next_p, call_depth + 1);
            ...
            if (!found_trial) {
                if (use_pm1) {
                    ...
                    if (verbose) std::cout << "pollard_pm1(n=" << n << ", B=" << low << ", seed=" << high_ << ") returned c=" << c << std::endl;
                    if (c) {
                ...
                if (use_rho) {
                    ...
                    if (verbose) std::cout << "pollard_rho(n=" << n << ", retries=1, max_steps=" << low << ", seed=" << high_ << ") returned c = " << c << std::endl;
                    if (c) {
                        std::vector<long long> ps;
                        if (c < next_p * next_p || setprimes[c]) {
                            ps.push_back(c);
    ...
    T pollard_pm1(T n, int B = 10, T a = 2, int retries = 0, unsigned int seed = 1234) {
        if (verbose) std::cout << "pollard_pm1(n=" << n << ", B=" << B << ", a=" << a << ", retries=" << retries << ", seed=" << seed << ")" << std::endl;
    ...
    T pollard_rho(T n1, T s = 2, T a1 = 1, int retries = 5, unsigned int seed = 1234, int64_t max_steps = 0) {
        if (verbose) std::cout << "pollard_rho(n1=" << n1 << ", s=" << s << ", a1= " << a1 << ", retries=" << retries << ", seed=" << seed << ", max_steps=" << max_steps << ")" << std::endl;
...
t1 = time.time()
divisors.is_prime(11)
divisors.is_prime(13033927)
dt = time.time() - t1
...
def time_sympy(ary):
    ...
    dt = time.time() - t1
    if dt < 60:
        print(f"sympy.divisors() {len(ary):,} in {round(dt, 2):.2f} seconds")
    else:
        print(f"sympy.divisors() {len(ary):,} in {round(dt/60, 2):.2f} minutes")

def time_divisors(ary):
    t1 = time.time()
    for i in range(0, len(ary)):
        sd = divisors.divisors(ary[i])
    dt = time.time() - t1
    if dt < 60:
        print(f"divisors.divisors() {len(ary):,} in {round(dt, 2):.2f} seconds")
    else:
        print(f"divisors.divisors() {len(ary):,} in {round(dt/60, 2):.2f} minutes")
...
1048576
sympy.ntheory.divisors(): 64.52
sympy_ntheory_factor.divisors(): 6.38
divisors.divisors(): 8.77
